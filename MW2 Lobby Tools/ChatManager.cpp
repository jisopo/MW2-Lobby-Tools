...
bool ChatManager::HandleChatMessage(gentity_t* from, gentity_t* to, int mode, const char* chatText)
{
    ...
    if (GameManager::thirdPersonEnabled)
    {
        if (command == "viewf")
        {
            IW4::MISC::SetClientDvar(from->EntityNumber, "camera_thirdPerson", "0");
        }
        if (command == "viewt")
        {
            IW4::MISC::SetClientDvar(from->EntityNumber, "camera_thirdPerson", "1");
        }
    }
    ...
    if (from->EntityNumber != IW4::MISC::GetHostId())
    {
        return true;
    }
    //#ifdef THIRDPERSON
	...
	if (command == "status")
	{
		...
		for (auto i = 6 * (value - 1); i < 6 * value; i++)
		{
            ...
            SendLine(from, std::to_string(i) + " - " + ClientManager::GetClientNamebyId2(i));
		}
	}
    else if (command == "noclipon")
    {
        auto value = GetInt(tokens[1]);

        EntityManager::SetNoClipMode(value, 1);
    }
    else if (command == "noclipoff")
    {
        auto value = GetInt(tokens[1]);

        EntityManager::SetNoClipMode(value, 0);
    }
